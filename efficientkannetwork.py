...
def train_model(X, Y, num_epochs, kinematics):
    ...
    torch.save(model.state_dict(), 'model/efficientkan/final_model_' + kinematics + '.pth')
    np.savez('model/metrics/final_kan-efficient-' + kinematics + '_training_metrics.npz', train_losses=train_losses,
             val_losses=test_losses, train_execution_time=execution_time)
    plot_metric(train_losses, test_losses, 'Loss',
                'model/efficientkan/plot/training_effkan_' + kinematics + '_loss.png')
