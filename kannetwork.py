...
def train_final_model(X, Y, config_file, kinematics='inverse'):
    ...
    model = KAN(width=width, grid=config['grid'], k=config['k'], seed=config['seed'])
    ...
    plot_metric(history['train_loss'], history['test_loss'], 'Loss',
                'model/kan/training_kan_LBFGS' + kinematics + '_loss.png')
    ...
    #     plot_metric(history['train_loss'], history['test_loss'], 'Loss',
    #                 'model/kan/training_kan_' + kinematics + '_grid_' + str(value) + '_loss.png')
    ...
    plt.savefig('model/kan/network_architecture_prepruning_LBFGS' + kinematics + '.png')
    plt.close()
    ...
    plt.figure(figsize=(15, 20))
    model.plot(beta=10)  # beta controlla la risoluzione del grafo
    plt.savefig('model/kan/network_architecture_postpruning_LBFGS' + kinematics + '.png')
    plt.close()

    # Salva il modello finale
    model.saveckpt(path='model/kan/final_model_kanLBFFGS_' + kinematics)
...
def inference(X, output_size, kinematics):
    ...
    model = model.loadckpt('model/kan/final_model_kanLBFFGS_' + kinematics)
    #
...
def main():
    ...
    kinematics = 'inverse'
    X, Y = read_data("dataset.txt", kinematics)
    # grid_search_kan(X[:10000], Y[:10000], param_grid)
